...
// Run with:
//   node --import ./tests/helpers/registerThreeStub.js --test tests/test_ui_helpers.js
//
...
test("mergeVfxProfile falls through to the generic numeric branch for a numeric glow_hex", () => {
  // Pinning current behavior: the numeric-field `else if` branch runs for
  // every DEFAULT_VFX key including glow_hex, so a numeric glow_hex passes
  // the finite-number check and is assigned as-is instead of being rejected
  // like other malformed glow_hex values.
